...
Comme expliquer ci-dessus, nousa avons décompensé la partie en trois temps :
...
- tout d'abord, nous avons considérer qu'il n'est intéressant de se splitter qu'en vu d'attaquer des groupes de taille inférieure. Par conséquent on n'explore les possibilités de split qu'à partir du moment où nous avons moins de groupe qu'il n'y a de groupes humains ou adverses présents. Qui plus est, même dans ce cas, le split n'est considéré que lorsqu'il y a au moins deux groupes présentant des tailles respectivement inférieures à celles des sous-groupes ainsi formés. 
- nous avons ensuite considérer qu'un split se faiser toujours en vu d'un objectif. Ainsi nous forçons ensuite le placement des groupes splittés plutôt que de considérer l'ensemble des combinaisons de 2 parmi 8 en ciblant les deux groupes de taille inférieures les plus proches de nos troupes.
- nous avons considérer que tous les groupes résultant des split étaient obligés de bouger au moins du temps qu'il reste des humains. En effet à ce stade, on est dans une situation de course de sorte qu'a priori l'immobilisme n'apporte rien, alors que cette hypothèse permet de gagner un temps de calcul précieux.
...
- des difficultée à expliquer certains comportements a priori lorsqu'on attend des profondeurs plus élevées 
