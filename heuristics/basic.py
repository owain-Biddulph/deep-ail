...
def simple_score(state, all_occupied_tile_us: List[List[int]], all_occupied_tile_other: List[List[int]],
    ...
    print(f'heuristic current state :  {current_state_score}')
    ...
    for tile_1 in all_occupied_tile_us:
        for tile_2 in all_occupied_tile_other:
            ...
            potential_score += distance_factor * get_proba_of_success(
                tile_1[2], state.our_species, tile_2[2], state.enemy_species)

    print(f'heuristic potential state {current_state_score}')
...
def distance(tile1_x, tile1_y, tile2_x, tile2_y):
    return max(abs(tile1_x - tile2_x), abs(tile1_y - tile2_y))
